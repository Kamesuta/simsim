...
public class TeleporterDiamondCraft extends Teleporter {
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8){
		...
		if (d3 >= 0.0D){
			if(flag){
				this.destinationCoordinateCache.add(j1, new TeleporterDiamondCraft.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if(this.worldServerInstance.getBlock(i, j, k - 1) == DiamondCraftBlock.TwilightPortalBlock){
				i4 = 3;
			}
